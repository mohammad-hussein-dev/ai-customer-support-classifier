...
class ModelEvaluator:
    ...
    def evaluate(
        ...
        console.print()
        console.rule(
            "[bold cyan]🤖 BANKING INTENT CLASSIFIER — EVALUATION REPORT", style="cyan"
        )
        console.print()
        ...
        console.print(
            Panel(
                summary,
                title="[bold green]Overall Performance",
                border_style="green",
                width=50,
            )
        )
    ...
    def _print_class_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> None:
        ...
        precisions = precision_score(
            y_true, y_pred, average=None, zero_division=0, labels=self.class_names
        )
        ...
        macro_f1 = f1_score(y_true, y_pred, average="macro", zero_division=0)
        table.add_row(
            "[bold]Macro Average[/bold]",
            "",
            "",
            f"[bold cyan]{macro_f1:.4f}[/bold cyan]",
            "",
            "",
            style="bold",
        )
    ...
    def _print_error_analysis(
        ...
        for (true_lbl, pred_lbl), errs in sorted_pairs:
            ...
            title = (
                f"[bold red]{true_lbl.replace('_', ' ').title()} → "
                f"{pred_lbl.replace('_', ' ').title()}[/bold red]"
            )
    ...
    def _print_business_analysis(self, y_true: np.ndarray, y_pred: np.ndarray) -> None:
        ...
        for intent in self.business_critical:
            ...
            table = Table(
                title=f"[bold red]{intent.replace('_', ' ').title()}[/bold red]",
                box=box.SIMPLE,
                border_style="red",
            )
            ...
            console.print(table)
            console.print(
                "[dim]False negatives = tickets that SHOULD have been "
                "routed here but weren't.[/dim]"
            )
            console.print(
                "[dim]These are the most costly errors for "
                "business-critical intents.[/dim]"
            )
            console.print()
    ...
    def _build_results_dict(
        ...
        results = {
            ...
            "confusion_matrix": confusion_matrix(
                y_true, y_pred, labels=self.class_names
            ).tolist(),
        }
