...
class BankingVisualizer:
    ...
    def plot_eda_dashboard(
        ...
        fig.text(
            0.5,
            0.955,
            f"Dataset: {len(df):,} tickets  |  "
            f"Intents: {n_intents}  |  "
            f"Avg length: {df['char_count'].mean():.0f} chars",
            ha="center",
            fontsize=10,
        ...
        ax1.set_title(
            "📊 Intent Distribution", fontweight="bold", pad=10, color=ACCENT_CYAN
        )
        ...
        bars2 = ax2.bar(
            ...
            ratios.values,
            color=[
                ACCENT_RED if r > 2 else ACCENT_YELLOW if r > 1.5 else ACCENT_GREEN
        ...
        ax2.set_xticklabels(
            [i.replace("_", " ").title()[:12] for i in ratios.index],
            rotation=45,
            ha="right",
            fontsize=8,
        )
        ...
        ax2.set_title(
            "⚖️ Imbalance Ratio", fontweight="bold", pad=10, color=ACCENT_YELLOW
        )
        ...
        parts = ax3.violinplot(
            [df[df[label_col] == intent]["char_count"].values for intent in intents],
            positions=range(n_intents),
            showmeans=True,
            showmedians=False,
        )
        ...
        ax3.set_xticklabels(
            [i.replace("_", " ").title()[:10] for i in intents],
            rotation=45,
            ha="right",
            fontsize=8,
        )
        ...
        ax3.set_title(
            "📝 Character Length", fontweight="bold", pad=10, color=ACCENT_GREEN
        )

        # ── 4. Word Count Box Plot ──
        ax4 = fig.add_subplot(gs[1, 1])
        bp = ax4.boxplot(
            [df[df[label_col] == intent]["word_count"].values for intent in intents],
            patch_artist=True,
            widths=0.6,
        )
        ...
        ax4.set_title(
            "🔤 Word Count Distribution", fontweight="bold", pad=10, color=ACCENT_PURPLE
        )
        ...
        ax5.set_title(
            "📏 Avg Length by Intent", fontweight="bold", pad=10, color=ACCENT_CYAN
        )
        ...
        ax6.set_title(
            "📈 Message Length Density (Top 3 Intents)",
            fontweight="bold",
            pad=10,
            color=ACCENT_GREEN,
        )
        ...
        fig.text(
            0.99,
            0.01,
            "Generated by BankingVisualizer | Dark Theme v2.0",
            ha="right",
            fontsize=7,
            color=TEXT_SECONDARY,
            alpha=0.5,
    ...
    def plot_evaluation_dashboard(
        ...
        """Combined evaluation figure: confusion matrix + per-class metrics
        + confidence.
        """
        ...
        ax1.set_yticklabels(
            [c.replace("_", " ").title() for c in class_names], fontsize=8
        )
        ...
        colors_bar = [
            ACCENT_GREEN if f >= 0.9 else ACCENT_YELLOW if f >= 0.75 else ACCENT_RED
            for f in sorted_f1s
        ]

        bars = ax2.barh(
            range(len(sorted_f1s)),
            sorted_f1s,
            color=colors_bar,
            height=0.6,
            edgecolor=DARK_BORDER,
        ...
        ax2.set_title(
            "📊 Per-Class F1 (Sorted)", fontweight="bold", pad=10, color=ACCENT_GREEN
        )
        ...
        ax3.set_title(
            "🎯 Precision vs Recall", fontweight="bold", pad=10, color=ACCENT_PURPLE
        )
    ...
    def plot_tfidf_terms(
        ...
        fig, axes = plt.subplots(
            n_rows, n_cols, figsize=(n_cols * 4.5, n_rows * 3.5), facecolor=DARK_BG
        )
        if n_classes == 1:
            axes = np.array([axes])
        axes = axes.flatten()
        ...
        fig.suptitle(
            "Top TF-IDF Terms by Intent (Logistic Regression Coefficients)",
            fontsize=16,
            fontweight="bold",
            color=TEXT_PRIMARY,
            y=0.98,
    ...
    def plot_confidence_analysis(
        ...
        ax1.axvline(
            ...
            linestyle="-",
            linewidth=2,
            label=f"Mean ({np.mean(max_conf):.3f})",
        )
        ax1.set_xlabel("Confidence", color=TEXT_SECONDARY)
        ax1.set_ylabel("Count", color=TEXT_SECONDARY)
        ax1.set_title(
            "📊 Confidence Distribution", fontweight="bold", color=ACCENT_CYAN
        )
        ...
        ax2.set_title(
            "📦 Confidence by True Intent", fontweight="bold", color=ACCENT_PURPLE
        )
        ...
        prob_true, prob_pred = calibration_curve(
            correct.astype(int), max_conf, n_bins=10
        )
        ax3.plot(
            prob_pred,
            prob_true,
    ...
    def plot_error_patterns(
        ...
        fig, axes = plt.subplots(
            (n_pairs + 1) // 2, 2, figsize=(16, n_pairs * 2.5), facecolor=DARK_BG
        )
        if n_pairs == 1:
            axes = np.array([[axes]])
        axes = axes.flatten()
        ...
        fig.suptitle(
            "Top Misclassification Patterns with Example Texts",
            fontsize=16,
            fontweight="bold",
            color=TEXT_PRIMARY,
        )
        ...
        for idx, (true_lbl, pred_lbl, count) in enumerate(top_pairs):
            ...
            ax.text(
                0.5,
                0.9,
                f"{true_lbl.replace('_', ' ').title()} → "
                f"{pred_lbl.replace('_', ' ').title()}",
                ha="center",
                va="top",
                fontsize=12,
                fontweight="bold",
            ...
            ax.text(
                0.5,
                0.78,
                f"{count} cases  |  Avg confidence: {np.mean(confidences):.3f}",
                ha="center",
                va="top",
                fontsize=9,
                color=TEXT_SECONDARY,
    ...
    def plot_baseline_comparison(
        ...
        ax.set_title(
            "📈 Baseline vs ML Model Performance",
            fontsize=14,
            fontweight="bold",
            color=ACCENT_CYAN,
        )
        ...
        if ml_f1 > baseline_f1:
            ...
            ax.annotate(
                ...
                xy=(1, ml_f1),
                xytext=(0.5, (baseline_f1 + ml_f1) / 2),
